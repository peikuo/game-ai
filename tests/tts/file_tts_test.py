...
def process_file_with_tts(file_path, voice_id=None, cleanup=True):
    ...
    for i, chunk in enumerate(chunks):
        ...
        success = tts_manager.speak_monologue(
            chunk, voice_id=voice_id, cleanup=cleanup)
        if not success:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert text file content to speech using TTS"
    )
    parser.add_argument("file", help="Path to the text file to read")
    parser.add_argument(
        "--voice",
        help="Voice ID to use (e.g., 'zf_001' for female, 'zm_010' for male)")
    parser.add_argument(
        "--keep-files",
        action="store_true",
    ...
    # Process the file
    success = process_file_with_tts(
        args.file, voice_id=voice_id, cleanup=not args.keep_files
    )
