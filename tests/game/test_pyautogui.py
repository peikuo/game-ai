...
def create_test_window():
    ...
    try:
        ...
        ttk.Button(
            frame,
            text="Click Me!",
            command=increment_counter).pack(
            pady=10)
        ...
        ttk.Label(
            frame,
            text="Move your mouse here and click\nto test PyAutoGUI functionality",
        ).pack(
            pady=10)
        ...
        logger.info(
            "Test window created. Close the window when testing is complete.")
        root.mainloop()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Test PyAutoGUI functionality")
    parser.add_argument(
        "--window",
        action="store_true",
        help="Create a test window to click on")
    parser.add_argument(
        "--click",
        action="store_true",
        help="Perform actual clicks (use with caution)")
