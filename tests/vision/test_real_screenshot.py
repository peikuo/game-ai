...
def create_game_screenshot(width=1920, height=1080):
            ...
            for k in range(8):
                if random.random() > 0.3:  # Some windows are dark
                    ...
                    if wx < x + w - 10 and wy < int(height * 0.7) - 10:
                        draw.rectangle(
                            [(wx, wy), (wx + 20, wy + 25)], fill=(255, 255, 200)
                        )

    # Add UI elements (HUD-like elements)

    # Health bar
    draw.rectangle([(50, 50), (300, 80)], fill=(
        0, 0, 0), outline=(255, 255, 255))
    draw.rectangle([(55, 55), (55 + 240, 75)], fill=(255, 50, 50))

    # Mini-map
    draw.rectangle(
        [(width - 220, 50), (width - 50, 220)],
        fill=(30, 30, 30),
        outline=(255, 255, 255),
    )
    ...
    for i in range(10):
        ...
        draw.ellipse([(mx, my), (mx + dot_size, my + dot_size)],
                     fill=dot_color)
    ...
    for i, item in enumerate(menu_items):
        y_pos = 300 + i * 40
        # Text background
        draw.rectangle([(width - 150, y_pos), (width - 20,
                                               y_pos + 30)], fill=(50, 50, 50, 128))
        # We can't easily draw text, so simulate with lines
        draw.line(
            [(width - 130, y_pos + 15), (width - 40, y_pos + 15)],
            fill=(255, 255, 255),
            width=2,
        )
    ...
    # Character body
    draw.ellipse(
        [(center_x - 50, center_y - 100), (center_x + 50, center_y + 100)],
        fill=(200, 150, 100),
    )
    # Character head
    draw.ellipse(
        [(center_x - 30, center_y - 160), (center_x + 30, center_y - 100)],
        fill=(200, 150, 100),
    )
...
def main():
    ...
    print(
        f"Size reduction vs PNG: {(1 - compressed_size/original_size) * 100:.2f}%")
    print(
        f"Size reduction vs JPEG (q=95): {(1 - compressed_size/jpeg_size) * 100:.2f}%"
    )
    print(
        f"Size reduction vs JPEG (q=40): {(1 - compressed_size/jpeg_low_size) * 100:.2f}%"
    )
