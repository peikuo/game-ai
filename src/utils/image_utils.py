...
def optimize_image(
    ...
    if optimize_colors and img.mode in ("RGB", "RGBA"):
        ...
        try:
            ...
            if width * height > 1000000:  # For images larger than 1 megapixel
                logger.info(
                    f"Skipping color optimization for large image ({width}x{height})"
                )
            ...
            if unique_colors < 256:
                ...
                logger.debug(
                    f"Converted image to palette mode (unique colors: {unique_colors})"
                )
            else:
    ...
    original_bytes = (
        width * height * (4 if img.mode == "RGBA" else 3)
    )  # Approximate size
    ...
    if (
        ...
        and not stats.get("color_optimized", False)
    ):
    ...
    stats["compression_ratio"] = (
        original_bytes / optimized_bytes if optimized_bytes > 0 else 0
    )
...
def encode_image_to_base64(
    ...
    try:
        ...
        # Estimate original uncompressed size (assuming 3 bytes per pixel for
        # RGB)
        original_bytes = original_pixels * (4 if img.mode == "RGBA" else 3)
        original_size_kb = original_bytes / 1024

        logger.info(
            f"Original image: {original_width}x{original_height} pixels, ~{original_size_kb:.2f} KB uncompressed"
        )
        ...
        # Always try PNG compression (good for screenshots with text/UI
        # elements)
        ...
        if not has_transparency:
            ...
            # Use quality=40 for game screenshots (good balance for game
            # visuals)
        ...
        size_reduction_percent = (
            1 - (compressed_size_bytes / original_bytes)) * 100

        # Log the image size and compression details
        logger.info(
            f"Before compression: {original_width}x{original_height} pixels, ~{original_size_kb:.2f} KB"
        )
        logger.info(
            f"After compression: {format_used}, {compressed_size_kb:.2f} KB, "
