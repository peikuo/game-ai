...
from src.utils.config_loader import PROJECT_ROOT, load_api_config, load_config
...
class WindowDetector:
    ...
    def _load_prompt_templates(self) -> Dict[str, str]:
        ...
        if window_detection_prompt:
            ...
            logger.warning(
                "No window detection prompt found in config, using fallback")
    ...
    def _detect_game_window_impl(
        ...
        logger.info(
            f"Original image dimensions: {original_size[0]}x{original_size[1]}")
        ...
        base64_image = encode_image_to_base64(
            screenshot, max_width=max_width, optimize=True
        )
        ...
        # For simplicity, we'll estimate the resized dimensions based on
        # max_width
        ...
        try:
            ...
            # Get the window region as a tuple from the model (based on resized
            # image)
            ...
            logger.info(
                f"Model detected window coordinates: x={window_loc.x}, y={window_loc.y}, width={window_loc.width}, height={window_loc.height}"
            )
            ...
            if original_size != resized_size:
                ...
                logger.info(
                    f"Original window coordinates: {model_window_region}")
            ...
            if window_loc.x < 0 or window_loc.y < 0:
                logger.warning(
                    f"Detected negative coordinates: ({window_loc.x}, {window_loc.y}). This may cause issues."
                )

            if window_loc.width <= 0 or window_loc.height <= 0:
                logger.warning(
                    f"Invalid window dimensions: width={window_loc.width}, height={window_loc.height}"
                )
                return None, None
            ...
            if window_loc.width < 100 or window_loc.height < 100:
                logger.warning(
                    f"Window dimensions seem too small: {window_loc.width}x{window_loc.height}"
                )
