...
class GameArea(BaseModel):
    ...
    player_area: Optional[List[int]] = Field(
        None, description="[x, y, width, height]")
    controls: Optional[List[int]] = Field(
        None, description="[x, y, width, height]")
    # Allow additional areas to be dynamically added
    additional_areas: Optional[Dict[str, List[int]]] = Field(
        None, description="Additional game areas"
    )
...
class ImageAnalyzer:
    ...
    def __init__(
        ...
        self.current_game = game_config.get(
            "name", "vanity_fair"
        )  # Use the game_name passed from main.py
        logger.info(
            f"Initializing image analyzer for game: {self.current_game}")
    ...
    def analyze(
        ...
        Args:
            ...
            region_name (str, optional): Name of the region to analyze
        ...
        try:
            ...
            # Load prompt from config based on analysis_type
            prompt = self.game_prompts.get(
                analysis_type, "Describe what you see in this game screenshot."
            )
            if prompt == "Describe what you see in this game screenshot.":
                logger.warning(
                    f"⚠️ USING FALLBACK PROMPT - No {analysis_type} prompt found in config for game: {self.current_game}"
                )
                logger.warning(
                    f"Available prompts in config: {list(self.game_prompts.keys())}"
                )
            else:
                logger.info(
                    f"Using game-specific prompt for {analysis_type} from config"
                )

            # Validate that the prompt isn't empty or too basic
            if len(prompt) < 50:
                logger.warning(
                    f"Prompt seems too short (only {len(prompt)} chars). This may not provide enough context for good analysis."
                )
    ...
    # The _analyze_with_ollama and _analyze_with_qwen methods have been
    # replaced by the centralized model_call.py
    ...
    def _parse_model_response(self, response_text):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Successfully parsed response as GameAnalysis: {len(json_data)} fields found"
                )
                ...
                if game_analysis.action_analysis.simple:
                    logger.info(
                        f"Simple action detected: {game_analysis.action_analysis.action.type} at position {game_analysis.action_analysis.action.position if game_analysis.action_analysis.action.position else 'N/A'}"
                    )
                else:
                    logger.info(
                        f"Complex action: {game_analysis.action_analysis.explanation}"
                    )
                ...
                logger.warning(
                    f"Error validating JSON with Pydantic model: {str(e)}")
    ...
    def _parse_text_response(self, response_text):
        ...
        logger.warning(
            "Falling back to text-based parsing since response is not JSON")

        # Create a structure similar to what we expect from JSON but built from
        # text parsing
        result = {
        ...
        for line in lines:
            ...
            if any(kw in line.lower() for kw in element_keywords) and any(
        ...
        # If we found no UI elements but have action coordinates, create a
        # default
        action = result["action_analysis"].get("action")
        if not result["ui_elements"] and action and action.get("position"):
